...
function* eachPiecePosition(index, rotation, xPos = 0, yPos = 0) {
  ...
  for (let y = 0; y < tetromino.size; y++) {
    for (let x = 0; x < tetromino.size; x++) {
      if (data[y][x]) {
        yield [xPos + x - halfSize, yPos + y - halfSize, tetromino];
      }
    }
  }
}
...
function drawPiece(display, index, rotation, left, top, xPos, yPos) {
  for (let [x, y, tetromino] of eachPiecePosition(
    index,
    rotation,
    xPos,
    yPos
...
function drawGhostPiece(display, index, rotation, left, top, xPos, yPos) {
  ...
  for (let [x, y, tetromino] of eachPiecePosition(
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const bg = palette256(...rgbpalette(value).map((v) => v * 0.5));
      const fg = palette256(...rgbpalette(value).map((v) => v * 0.15));
      // paletteGrayscale(value * 0.5);

      display.printText('░', { bg, fg });
    }
  }
}
...
function rgbpalette(x, fn = (x) => x) {
  return Array.from({ length: 3 }, (_, i) =>
    Math.floor(256 * Math.sin((x + i / 3.0) * Math.PI) ** 2)
  );
}
...
function drawBackground(display, top, right, bottom, left, tdiv, scale = 1.0) {
  ...
  for (let y = top + 1; y < bottom; y++) {
    for (let x = left + 1; x < right; x++) {
      ...
      display.setCursorPosition(x, y).putCell(char, {
        bg: palette256(...rgbpalette(value).map((v) => v * 0.5)),
      });
    }
  }
}
...
function drawGameOverScreen(display) {
  ...
  for (let y = 0; y < display.rows; y++) {
    for (let x = 0; x < display.cols; x++) {
      ...
      display.setCursorPosition(x, y).putCell(char, {
        bg: palette256(...rgbpalette(value).map((v) => v * alpha)),
      });
    }
  }
  ...
  for (let j = 0; j < yCount; j++) {
    for (let i = 0; i < xCount; i++) {
      const r = ((j * yCount + i) / (yCount * xCount)) * 200.0;
      const rotation = Math.floor(t / 2.0 + r) % 4;

      const yPos =
        Math.floor((display.rows / yCount) * j + display.rows / 8) +
        (Math.floor(
          ((Math.sin((t / 20 + i / 16.0 + j / 16) * Math.PI) + 1.0) / 2.0) *
            display.rows -
            1
        ) %
          display.rows);
      ...
      const xPos = Math.floor(
        ((i / xCount) * (display.cols - offset / 2) + offset / 2) / 2 +
          offset / 2
      );
      ...
      for (let [x, y] of eachPiecePosition(i, rotation, xPos, yPos)) {
        ...
        try {
          display.setCursorPosition(x * 2, y).printText('  ', { bg });
        } catch (e) {
          log(e.message);
  ...
  const y = Math.floor(
    (display.rows * 2) / 3 - (GAME_OVER_TEXT.length * 2) / 3
  );
  const y2 = Math.floor((display.rows * 1) / 3 - (TITLE_TEXT.length * 1) / 3);
  printTitle(display, TITLE_TEXT, y2);
  printTitle(display, GAME_OVER_TEXT, y);
  ...
  */
  ...
  display
    .setCursorPosition(Math.floor(display.cols / 2 - text.length / 2), textTop)
    .printText(text, { fg: 15 });
}

function printTitle(display, lines, textTop) {
