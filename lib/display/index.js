...
function updateColor(prevFg, prevBg, newFg, newBg) {
  ...
  if (!equalFg) {
    result.push(ANSI.fgColor(newFg));
  }
  if (!equalBg) {
    result.push(ANSI.bgColor(newBg));
  }
...
class Display {
  ...
  stop() {
    ...
    process.stdout.write(
      '\x1b[0m' + ANSI.cursorVisible(true) + ANSI.disableAlternateScreenBuffer()
    );
  ...
  *changedRows() {
    const definitelyHasChanged =
      !this.prevBuffer || this.prevBuffer.length !== this.buffer.length;
  ...
  handleResize(e) {
    if (
      process.stdout.columns === this.cols &&
      process.stdout.rows === this.rows
    ) {
      return;
    }
