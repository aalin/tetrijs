...
function createGrid(width, height) {
  return Array.from({ length: width * height }).fill(0);
...
class Grid {
  ...
  getGhostPosition() {
    for (let y = this.pieceY + 1; y < this.pieceY + this.height + 1; y++) {
  ...
  draw(display) {
    ...
    if (this.nextPieceId !== null) {
      graphics.drawPiece(display, this.nextPieceId, 0, 3, 7, 0, 0);
    }
  }
}
